          ...
          child: GlowingOverscrollIndicator(
            ...
            child: WatchBoxBuilder(
              ...
              builder: (context, mybox) {
                return mybox.isEmpty?Column(children: [
                  ...
                  , ],
                ):ListView.builder(itemCount: mybox.length, itemBuilder: (BuildContext context, int index) {
                ...
                if(elt.runtimeType==Note){
                  ...
                  return Padding(
                    ...
                    child: Slidable(
                      startActionPane: ActionPane(motion: DrawerMotion(), children: [SlidableAction(icon: CupertinoIcons.trash,backgroundColor: Colors.deepOrange.shade900,onPressed: (BuildContext context) {
                        setState(() {
                      ...
                      child: Card(
                        ...
                        child: TextButton(style: ButtonStyle(shape: MaterialStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(20.0)))),overlayColor: MaterialStateProperty.all(Colors.teal[100])),
                          ...
                          child: Container(
                              ...
                              child : Stack(children: [ListTile(title: Text(note.title,style: TextStyle(color: Colors.lightGreen[200]),),subtitle: Text(note.content.length<25?note.content:note.content.substring(0,25)+"...", style: TextStyle(color: Colors.white38)),),Container(padding: EdgeInsets.only(top: 25),alignment: Alignment(0.789,1),child: Icon(Icons.note_add,color: Colors.lightGreenAccent,))])
                          ),
                        ),
                      ),
                    ),
                  ...
                  return Padding(
                    padding: EdgeInsets.only(top: 30.0,left: 10.0,right: 10.0,bottom: 30.0),
                    child: Slidable(
                      startActionPane: ActionPane(motion: DrawerMotion(), children: [SlidableAction(icon: CupertinoIcons.trash,backgroundColor: Colors.deepOrange.shade900,onPressed: (context) {
                        setState(() {
                          mybox.deleteAt(index);
                        });
                      },)],),
                      child: Card(
                        ...
                        child: TextButton(style: ButtonStyle(shape: MaterialStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(20.0)))),overlayColor: MaterialStateProperty.all(Colors.teal[100])),
                          onPressed: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>List_Page(items: list,index: index,)));
                          },
                          child: Container(
                              margin: EdgeInsets.only(top: 5.0,bottom: 5.0),
                              padding:EdgeInsets.all(22.75),
                              child : Stack(children: [ListTile(title: Text(list.title,style: TextStyle(color: Colors.lightGreen[200]),),subtitle: Text("To-Do-List "+"...", style: TextStyle(color: Colors.white38)),),Container(padding: EdgeInsets.only(top: 25),alignment: Alignment(0.789,1),child: Icon(Icons.list,color: Colors.lightGreenAccent,))])
                          ),
                        ),
                      ),
                    ),
